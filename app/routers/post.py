...
def get_posts(db: Session = Depends(get_db), user: int = Depends(oauth2.get_current_user), limit: int=10, skip:int = 0, search:Optional[str] = ""):
    ...
    posts = db.query(models.Post, func.count(models.Vote.post_id).label("votes")).join(models.Vote, models.Vote.post_id == models.Post.post_id, isouter = True).group_by(models.Post.post_id).filter(models.Post.title.contains(search)).limit(limit).offset(skip).all()
    return posts

@router.post("/", status_code=status.HTTP_201_CREATED, response_model=schemas.PostResponse)
def create_post(post: schemas.CreatePost, db: Session = Depends(get_db), user: int = Depends(oauth2.get_current_user)):
...
def get_post(id : int, db: Session = Depends(get_db), user: int = Depends(oauth2.get_current_user)):
    ...
    post = db.query(models.Post, func.count(models.Vote.post_id).label("votes")).join(models.Vote, models.Vote.post_id == models.Post.post_id, isouter = True).group_by(models.Post.post_id).first()
    if not post:
...
def update_post(id: int, post: schemas.CreatePost, db: Session = Depends(get_db), user: int = Depends(oauth2.get_current_user)):
    ...
    if not post1:
        raise HTTPException(status_code = status.HTTP_404_NOT_FOUND, detail = f"Post with id: {id} not Exists")
    if post1.user_id != user.user_id:
        raise HTTPException(status_code = status.HTTP_403_FORBIDDEN, detail = f"Not Authorized")
    # post_query.update({'title' : 'updated', 'content' : 'updated'}, synchronize_session=False)
